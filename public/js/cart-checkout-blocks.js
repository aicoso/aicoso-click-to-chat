...
(function() {
    ...
    function insertWhatsAppButton() {
        ...
        if (isCartPage && ctc_chat_block_params.cart_enabled === '1') {
            pageType = 'cart';
            isEnabled = true;
            position = ctc_chat_block_params.cart_position || 'after_cart_table';
        } else if (isCheckoutPage && ctc_chat_block_params.checkout_enabled === '1') {
            pageType = 'checkout';
            isEnabled = true;
            position = ctc_chat_block_params.checkout_position || 'after_payment';
        }
    ...
    function createButtonHtml(pageType) {
        const buttonText = ctc_chat_block_params.button_text || 'Order via WhatsApp';
        const bgColor = ctc_chat_block_params.bg_color || '#25D366';
        const textColor = ctc_chat_block_params.text_color || '#ffffff';
        const showIcon = ctc_chat_block_params.show_icon === '1';
        const whatsappUrl = ctc_chat_block_params.whatsapp_url || '#';
        ...
        html += `
                    <span class="ctc-chat-button-text">${buttonText}</span>
                </a>
            </div>
        `;
